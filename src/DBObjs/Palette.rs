...
pub const PALETTE_ATTR: DBObjTypeAttribute = DBObjTypeAttribute {
    rust_type_name: "Palette",
    dat_file_type: DatFileType::Portal,
    db_obj_type: DBObjType::Palette,
...
impl IUnpackable for Palette {
    fn unpack(&mut self, reader: &mut DatBinReader<'_>) -> bool {
        ...
        for _ in 0..num_colors {
            self.colors.push(reader.read_item::<ColorARGB>());
        }
        true
    }
}
impl IPackable for Palette {
    fn pack(&self, writer: &mut DatBinWriter<'_>) -> bool {
        let _ = self.base.pack_with_flags(writer, DBObjHeaderFlags::HasId);
        writer.write_i32(self.colors.len() as i32);
