...
use crate::{
    ...
    Types::{
        ...
        MotionCommandData::MotionCommandData,
        MotionData::MotionData,
    },
};

pub const MOTION_TABLE_ATTR: DBObjTypeAttribute = DBObjTypeAttribute {
    rust_type_name: "MotionTable",
    dat_file_type: DatFileType::Portal,
    db_obj_type: DBObjType::MotionTable,
...
impl IUnpackable for MotionTable {
    fn unpack(&mut self, reader: &mut DatBinReader<'_>) -> bool {
        ...
        for _ in 0..num_style_defaults {
            self.style_defaults.insert(
                MotionCommand::from(reader.read_u32()),
                MotionCommand::from(reader.read_u32()),
            );
        }
        ...
        for _ in 0..num_cycles {
            ...
            let val = reader.read_item::<MotionData>();
            self.cycles.insert(key, val);
        }
        ...
        for _ in 0..num_modifiers {
            let key = reader.read_i32();
            let val = reader.read_item::<MotionData>();
            self.modifiers.insert(key, val);
        }
        let num_links = reader.read_u32() as usize;
        self.links.clear();
        for _ in 0..num_links {
            let key = reader.read_i32();
            let val = reader.read_item::<MotionCommandData>();
            self.links.insert(key, val);
        }
        true
    }
}
impl IPackable for MotionTable {
    fn pack(&self, writer: &mut DatBinWriter<'_>) -> bool {
        let _ = self.base.pack_with_flags(writer, DBObjHeaderFlags::HasId);
        writer.write_u32(self.default_style.0);
        writer.write_u32(self.style_defaults.len() as u32);
        for (k, v) in &self.style_defaults {
            writer.write_u32(k.0);
            writer.write_u32(v.0);
        }
        writer.write_u32(self.cycles.len() as u32);
        for (k, v) in &self.cycles {
            writer.write_i32(*k);
            let _ = v.pack(writer);
        }
        writer.write_u32(self.modifiers.len() as u32);
        for (k, v) in &self.modifiers {
            writer.write_i32(*k);
            let _ = v.pack(writer);
        }
        writer.write_u32(self.links.len() as u32);
