...
use crate::{
    ...
    Types::{DBObj::{DBObj, DBObjBase}, PhysicsScriptData::PhysicsScriptData},
};

pub const PHYSICS_SCRIPT_ATTR: DBObjTypeAttribute = DBObjTypeAttribute { rust_type_name: "PhysicsScript", dat_file_type: DatFileType::Portal, db_obj_type: DBObjType::PhysicsScript, header_flags: DBObjHeaderFlags::HasId, first_id: 0x33000000, last_id: 0x3300FFFF, mask_id: 0x00000000 };
...
impl DBObj for PhysicsScript {
    ...
    fn db_obj_type(&self) -> DBObjType { DBObjType::PhysicsScript }
...
impl IUnpackable for PhysicsScript {
    fn unpack(&mut self, reader: &mut DatBinReader<'_>) -> bool {
        ...
        for _ in 0..count {
            self.script_data.push(reader.read_item::<PhysicsScriptData>());
        }
        true
    }
}
...
impl IDBObj for PhysicsScript {
    fn db_obj_type_attr() -> &'static DBObjTypeAttribute where Self: Sized { &PHYSICS_SCRIPT_ATTR }
    fn db_obj_type(&self) -> DBObjType { DBObjType::PhysicsScript }
    fn id(&self) -> u32 { self.base.id }
    fn set_id(&mut self, id: u32) { self.base.id = id; }
