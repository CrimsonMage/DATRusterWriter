...
| `DatReaderWriter/Lib/IO/BlockAllocators/IDatBlockAllocator.cs` | `src/Lib/IO/BlockAllocators/IDatBlockAllocator.rs` | Partial | Expanded to include explicit write-capable allocator operations alongside the existing read contract, and async read/write block wrapper entry points are now exercised by focused B-tree tests |
...
| `DatReaderWriter/Lib/IO/DatBTree/DatBTreeReaderWriter.cs` | `src/Lib/IO/DatBTree/DatBTreeReaderWriter.rs` | Partial | Read-path lookup, enumeration, caching, and range traversal remain tested, explicit synchronous write-side insert/update/delete behavior now includes flat-index coherence coverage for insert and delete, and async wrapper entry points for file lookup/insert/delete are now both exposed and exercised; deeper async mutation parity still remains pending |
...
| DatReaderWriter/Types/HashTable.cs | src/Types/HashTable.rs | Partial | Explicit read/write hash-table wrapper now uses the shared hash-table helpers for bucket sizing and AC string-key hashing, and is covered by focused roundtrip plus packed-order tests; full generic parity with every reference generic case still remains |
...
| `DatReaderWriter/Generated/Types/AnimationHook.generated.cs` and hook variants | `src/Types/AnimationHook.rs` | Partial | Read-side hook family collapsed into one Rust enum; known hook variants are covered by focused roundtrip tests, and unknown hook variants now preserve their trailing raw payload bytes for stable roundtrips |
...
| `DatReaderWriter/DatDatabase.cs` | `src/DatDatabase.rs` | Partial | Raw file entry lookup, byte/decompression read support, typed `try_get<T>()`, typed id enumeration including masked and special-routed cell DBObjs, allocator selection for both read-only and read-write access, explicit synchronous typed/raw write helpers with compression plus template-based flags/version/iteration reuse, typed in-memory file caching with clear-cache parity, and explicit async read/cache/file-byte/write wrapper methods are now covered by focused database and collection tests; deeper allocator/tree async parity still remains |
| `DatReaderWriter/DatCollection.cs` | `src/DatCollection.rs` | Partial | Typed `try_get<T>()`, typed `get_cached<T>()`, portal/high-res fallback, typed id enumeration, portal-backed master-property resolution for local property-driven reads, explicit typed write delegation including template-based metadata reuse, upstream-style `Iteration` collection guards, and async typed read/cache/write entry points are now ported and tested; deeper allocator/tree async parity still remains |
...
| `DatReaderWriter/PortalDatabase.cs` | `src/PortalDatabase.rs` | Verified | Concrete wrapper with header validation, typed read/write delegation, and convenience accessors for `MasterProperty` and `Region` validated against retail DATs |
| `DatReaderWriter/LocalDatabase.cs` | `src/LocalDatabase.rs` | Verified | Concrete wrapper with header validation plus explicit typed read/write delegation |
...
| `DatReaderWriter.Tests/IO/DatBTree/DatBTreeReaderWriterTests.cs` | `tests/btree_tests.rs` | Partial | Mock-allocator coverage now includes read traversal plus write-side insert, replace, root-split, flat-index coherence, leaf delete, root-collapse, and async allocator/B-tree wrapper behavior |
...
| `DatReaderWriter/Generated/Types/LayerStage.generated.cs` | `src/Types/LayerStage.rs` | Verified | Explicit layer-stage payload now mirrors the generated file shape and is covered by focused roundtrip tests |
| `DatReaderWriter/Generated/Types/LayerModifier.generated.cs` | `src/Types/LayerModifier.rs` | Verified | Explicit empty base layer-modifier surface now mirrors the generated file shape and is covered by focused roundtrip tests |
| `DatReaderWriter/Generated/Types/LM_UVTranslate.generated.cs` | `src/Types/LM_UVTranslate.rs` | Verified | Explicit UV-translate modifier now mirrors the generated file shape and is covered by focused roundtrip tests |
| `DatReaderWriter/Generated/Types/LM_UVRotate.generated.cs` | `src/Types/LM_UVRotate.rs` | Verified | Explicit UV-rotate modifier now mirrors the generated file shape and is covered by focused roundtrip tests |
| `DatReaderWriter/Generated/Types/LM_UVScale.generated.cs` | `src/Types/LM_UVScale.rs` | Verified | Explicit UV-scale modifier now mirrors the generated file shape and is covered by focused roundtrip tests |
| `DatReaderWriter/Generated/Types/LM_UVTransform.generated.cs` | `src/Types/LM_UVTransform.rs` | Verified | Explicit UV-transform modifier now mirrors the generated file shape and is covered by focused roundtrip tests |
| `DatReaderWriter/Generated/Types/BannedPatterns.generated.cs` | `src/Types/BannedPatterns.rs` | Verified | Explicit banned-pattern hash-table payload now mirrors the generated file shape and is covered by focused roundtrip tests |
| `DatReaderWriter/Generated/Types/ShaderResourceEntry.generated.cs` | `src/Types/ShaderResourceEntry.rs` | Verified | Explicit shader-resource entry payload now mirrors the generated file shape and is covered by focused roundtrip tests |
| `DatReaderWriter/Generated/Types/SubPalette.generated.cs` | `src/Types/SubPalette.rs` | Verified | Explicit split-out sub-palette wrapper now mirrors the generated file shape and is covered by focused roundtrip tests alongside the existing `ObjDesc` collapse |
| `DatReaderWriter/Generated/Types/TextureMapChange.generated.cs` | `src/Types/TextureMapChange.rs` | Verified | Explicit split-out texture-map-change wrapper now mirrors the generated file shape and is covered by focused roundtrip tests alongside the existing `ObjDesc` collapse |
| `DatReaderWriter/Generated/Types/SoundHook.generated.cs` | `src/Types/SoundHook.rs` | Verified | Explicit sound-hook wrapper now mirrors the generated file shape and is covered by focused roundtrip tests against the shared hook payload |
| `DatReaderWriter/Generated/Types/SoundTableHook.generated.cs` | `src/Types/SoundTableHook.rs` | Verified | Explicit sound-table-hook wrapper now mirrors the generated file shape and is covered by focused roundtrip tests against the shared hook payload |
| `DatReaderWriter/Generated/Types/AttackHook.generated.cs` | `src/Types/AttackHook.rs` | Verified | Explicit attack-hook wrapper now mirrors the generated file shape and is covered by focused roundtrip tests against the shared hook payload |
