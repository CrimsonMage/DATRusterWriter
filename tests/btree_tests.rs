...
impl MockBlockAllocator {
    fn new(root_block: i32, blocks: HashMap<usize, Vec<u8>>) -> Self {
        let mut header = DatHeader::new(DatFileType::Portal, 0, 2048, Some("test".to_string()), 1, 1, Uuid::nil(), 1);
...
impl IDatBlockAllocator for MockBlockAllocator {
    ...
    fn header(&self) -> &DatHeader { &self.header }
...
fn btree_try_get_file_and_range_walk_across_branches() {
    ...
    let all_ids: Vec<u32> = tree.all_files().unwrap().into_iter().map(|f| f.id).collect();
    assert_eq!(vec![10, 20, 30, 40, 50], all_ids);

    let range_ids: Vec<u32> = tree.get_files_in_range(15, 45).unwrap().into_iter().map(|f| f.id).collect();
...
fn btree_flat_index_preserves_sorted_range_results() {
    ...
    let ids: Vec<u32> = tree.get_files_in_range(1, 20).unwrap().into_iter().map(|f| f.id).collect();
